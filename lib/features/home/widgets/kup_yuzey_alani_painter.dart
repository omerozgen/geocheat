...
class KupYuzeyAlaniPainter extends StatelessWidget {
  final double size;
  const KupYuzeyAlaniPainter({super.key, this.size = 180});

  @override
  Widget build(BuildContext context) {
    return CustomPaint(
      size: Size(size, size),
      painter: _KupYuzeyAlaniPainter(),
    );
  }
}

class _KupYuzeyAlaniPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    // Ön yüzü taralı göster (yüzey alanı)
    ...
    _drawText(
      canvas,
      'Yüzey Alanı',
      Offset(frontTopLeft.dx + side / 4, frontTopLeft.dy + side / 2),
