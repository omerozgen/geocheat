...
class IkizkenarUcgenTabanAciPainter extends StatelessWidget {
  final double size;
  const IkizkenarUcgenTabanAciPainter({super.key, this.size = 180});

  @override
  Widget build(BuildContext context) {
    return CustomPaint(
      size: Size(size, size),
      painter: _IkizkenarUcgenTabanAciPainter(),
    );
  }
}

class _IkizkenarUcgenTabanAciPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    // Taban açılarına açı işareti
    _drawAngleArc(canvas, a, c, b, 22, Colors.red);
    _drawAngleArc(canvas, b, c, a, 22, Colors.red);
    _drawText(
      canvas,
      'α',
