...
class EskenarUcgenCevresiPainter extends StatelessWidget {
  final double size;
  const EskenarUcgenCevresiPainter({super.key, this.size = 180});

  @override
  Widget build(BuildContext context) {
    return CustomPaint(
      size: Size(size, size),
      painter: _EskenarUcgenCevresiPainter(),
    );
  }
}

class _EskenarUcgenCevresiPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    // Çevreyi vurgulayan ok
    _drawPerimeterArrow(canvas, a, b, Colors.red);
    // Etiket
    _drawText(
      canvas,
      'Çevre',
